...
export function ConnectionBanner({ backendError, gemini }: { backendError?: string | null; gemini?: GeminiStatus | null }) {
  if (backendError) {
    return (
      <div className="flex items-center gap-3 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-100">
        ...
        <span>Backend unavailable. Start FastAPI on port 8000.</span>
      </div>
    );
  }
  ...
  if (!gemini.configured) {
    return (
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
